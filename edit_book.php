...
<h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold text-blue-500 text-center mb-4 sm:mb-6">Edit Audiobook</h1>
    ...
    <form action="edit_book.php?id=<?php echo $book_id; ?>" method="post" enctype="multipart/form-data" class="space-y-4">
        ...
        <div class="flex flex-col sm:flex-row sm:items-start gap-4 min-w-0">
            <div class="flex-shrink-0 mx-auto sm:mx-0">
                 <p class="text-gray-400 mb-2 text-center sm:text-left">Current Cover</p>
                <img src="<?php echo htmlspecialchars($book['cover_image_path']); ?>" alt="Current Cover" class="w-24 h-24 object-cover rounded-md mx-auto sm:mx-0">
            </div>
            <div class="flex-grow min-w-0 w-full">
                <label for="cover_image" class="block text-gray-400 mb-2">Replace Cover Image (optional)</label>
                <input type="file" name="cover_image" id="cover_image" class="w-full text-white bg-gray-700 rounded-lg p-2 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
            </div>
        </div>
        <div class="flex flex-col sm:flex-row sm:items-start gap-4 min-w-0">
             <div class="flex-shrink-0 w-full sm:w-auto min-w-0">
                 <p class="text-gray-400 mb-2">Current Audio</p>
                 <audio controls class="w-full min-w-0 max-w-full">
                     <source src="<?php echo htmlspecialchars($book['audio_file_path']) . '?v=' . time(); ?>" type="audio/mpeg">
                     Your browser does not support the audio element.
                 </audio>
             </div>
             <div class="flex-grow min-w-0 w-full">
                <label for="audio_file" class="block text-gray-400 mb-2">Replace Audio File (optional)</label>
                <input type="file" name="audio_file" id="audio_file" class="w-full text-white bg-gray-700 rounded-lg p-2 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
            </div>
        </div>
