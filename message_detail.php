...
try {
    $sql = "SELECT m.*, u.is_deleted AS user_is_deleted, u.deleted_at AS user_deleted_at, m.user_deleted
            FROM messages m
    ...
    $is_deleted_user = !empty($message['user_is_deleted']) || !empty($message['user_deleted']);
    ...
    if ($message['status'] === 'pending') {
        try {
            ...
            if ($stmt_update->execute()) {
                ...
                $sql_re_fetch = "SELECT m.*, u.is_deleted AS user_is_deleted, u.deleted_at AS user_deleted_at, m.user_deleted
                                 FROM messages m
                                 LEFT JOIN users u ON m.user_id = u.id
                                 WHERE m.id = :id";
...
<div class="admin-page-inner admin-page-inner--medium">
        <div class="flex items-start gap-3 sm:gap-4 mb-4 sm:mb-6 min-w-0">
            <a href="admin_panel.php" class="text-blue-400 hover:text-blue-300 transition-colors shrink-0 mt-1">
                <i class="fas fa-arrow-left"></i> 
            </a>
                 <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-blue-500 break-anywhere min-w-0 flex flex-wrap items-center gap-2 sm:gap-3">
                <span>Message from <?php echo htmlspecialchars($message['username']); ?></span>
                <?php if ($is_deleted_user): ?>
                    <span class="inline-flex items-center px-2.5 py-0.5 text-xs sm:text-sm font-semibold rounded-full bg-red-600 text-white shrink-0 whitespace-nowrap mt-1 sm:mt-0">
                        Deleted User
                    </span>
                <?php endif; ?>
            </h1>
        ...
        <div class="bg-gray-800 p-4 sm:p-6 rounded-lg shadow-xl mb-6 sm:mb-8 min-w-0">
            <div class="space-y-4 break-anywhere">
                <?php if ($is_deleted_user): ?>
                    <p class="text-red-400"><span class="font-bold">Account Status:</span> This user account has been deleted.</p>
                <?php endif; ?>
                ...
                <div class="bg-gray-700 p-3 sm:p-4 rounded-lg overflow-x-auto">
                    <p class="whitespace-pre-wrap break-anywhere"><?php echo htmlspecialchars($message['message']); ?></p>
                </div>
                <p class="text-xs sm:text-sm text-gray-400 text-left sm:text-right">Sent on: <?php echo date("F j, Y, g:i a", strtotime($message['sent_at'])); ?></p>
            </div>
